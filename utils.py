...
def initializeGroups(groups : List['domain.Group'], techniques : List['domain.Technique']) -> None:
    ...
    for name, group in dfg:
        ...
        for row in group.itertuples():
            ...
            ttp = next((x for x in techniques if x.id == row.targetId), None)
            if ttp is None:
              print(f"Warning: targetId {row.targetId} not found in techniques. Skipping row.")
              continue
...
#sensitivity analysis rule generation for Table - XII
def generateSensitivityRules(cocTTPs: List[List['domain.Technique']], minSup=0.10, minConf=0.505):
    """
    Core rule generation logic used for sensitivity analysis and graph building.
    """
    # 1. Prepare transactions from Technique objects
    transactions = []
    for cases in cocTTPs:
        # Extract the technique ID (e.g., 'T1059') for processing
        transaction = [x.id for x in cases]
        transactions.append(transaction)

    # 2. Transform transactions to boolean array
    te = TransactionEncoder()
    te_ary = te.fit(transactions).transform(transactions)
    df = pd.DataFrame(te_ary, columns=te.columns_)

    # 3. Generate frequent itemsets using the sensitivity parameter 'minSup'
    frequent_itemsets = fpgrowth(df, min_support=minSup, use_colnames=True)
    
    # Check if any frequent itemsets were found before proceeding
    if frequent_itemsets.empty:
        return pd.DataFrame()

    # 4. Generate association rules using the sensitivity parameter 'minConf'
    rules = association_rules(frequent_itemsets, metric="confidence", min_threshold=minConf)
    
    # 5. Add metadata columns for analysis filtering
    if not rules.empty:
        rules['alen'] = rules['antecedents'].apply(lambda x : len(x)) 
        rules['clen'] = rules['consequents'].apply(lambda x : len(x)) 
